...
This document performs two method of feature selection in order to reduce the
number of proteins by selecting that ones with high biological variability.
The first method is based on the decomposition of variance into technical and
biological by nonparametric modeling the variance intensity as a function of
mean intensity. The latter uses the Hedges'G effect size statistics.
...
proteins, then subtract the variance of each proteins by their corresponding
fitted values, the remaining values are called biological variability.
The `modelGeneVar` function from `scran` package [@Aaron2016] is used.
...
These two extreme proteins are listed below. Their , which
could impact the fitted curve. Therefore, we will remove these cases from the
analysis.
...
# Effect size statistics {#sec-effect_size}
...
it means that their expression are regulated by physiological gene expression.
We use the Hedges' effect size statistic to do that. The statistics is defined
as follows
...
@fig-densities_hedges_g shows the density of the absolute value of Hedges'G
statistics according to the treatment. 
...
After computing the Hedges'G statistic we have to consider a threshold to
...
@fig-densities_hedges_g displays the total of proteins selected by group.
