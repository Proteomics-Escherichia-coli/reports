...
The report is organized as follows. @sec-data describe data import and management
using QFeatures package. Processing steps, namely filtering, imputation and
...
original paper and the steps performed.
...
`evidence.txt` file which combines all the information about identified peptides
...
In @fig-number_peptides we show the distribution of the number of peptides
used to aggregate at protein level. It is observe that most of the proteins 
are connected to 3-6 peptides.

```{r r-vis-number-peptides}
#| fig-width: 8
#| label: fig-number_peptides
#| fig-cap: Distribution of the peptides number matches with proteins. 
